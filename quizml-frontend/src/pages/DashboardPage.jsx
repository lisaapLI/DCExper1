        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-surface-container-high border-[3px] border-black px-4 py-1.5 neo-shadow-sm hidden sm:block">
              <span className="font-label-mono text-label-sm text-on-surface font-bold uppercase">
                {user.nama}
              </span>
            </div>
            <button
      ...
      <main className="max-w-5xl mx-auto px-margin-mobile md:px-margin-desktop py-xl space-y-xl">
        ...
        <section className="animate-fade-up animate-delay-100">
          <div className="flex items-center gap-xs mb-lg">
            ...
            <h2 className="font-display text-headline-md text-on-surface uppercase">
              Pilih Level
            </h2>
          </div>
                ...
                >
                  <div className="p-lg flex flex-col gap-md flex-1">
                    <div className="flex items-start justify-between">
                      ...
                      <span className="font-label-mono text-label-sm bg-black text-white px-2 py-0.5 font-black">
                        {lv.soal}
                      </span>
                    </div>
                    <div>
                      <h3
                        className={`font-display text-headline-lg ${lv.textColor} uppercase tracking-tight leading-none`}
                      >
                        {lv.label}
                      </h3>
                      <p
                        className={`font-body-md text-body-md ${lv.textColor} opacity-70 mt-xs leading-snug`}
                      >
                        {lv.desc}
                      </p>
                    </div>
                    <div className="border-t-[2px] border-black pt-xs flex items-center justify-between">
                      {best !== undefined ? (
                        <div>
                          <p className="font-label-mono text-[10px] text-black/60 uppercase font-black">
                            Best Score
                          </p>
                          <p
                            className={`font-display text-headline-md ${lv.textColor} font-black`}
                          >
                            {best}/100
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p
                            className={`font-display text-headline-md ${lv.textColor} opacity-40`}
                          >
                            —
                          </p>
                        </div>
        ...
        <section className="animate-fade-up animate-delay-200">
          <div className="flex items-center justify-between mb-lg">
            <div className="flex items-center gap-xs">
              ...
              <span className="font-label-mono text-label-sm text-on-surface-variant ml-2">
                ({history.length} sesi)
              </span>
            </div>
            {history.length > 0 && (
          ...
          {history.length === 0 ? (
            <div className="bg-surface-container border-[3px] border-black p-xl neo-shadow text-center">
              ...
              <p className="font-label-mono text-label-sm text-on-surface-variant mt-xs">
                Pilih level di atas untuk memulai!
              </p>
            </div>
          ) : (
            <div className="space-y-md">
              {history.map((h, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="bg-surface-container border-[3px] border-black neo-shadow flex items-center gap-md p-md"
                  >
                    {/* Level badge */}
                    <div
                      className={`${lv.accent} border-[2px] border-black px-3 py-1 neo-shadow-sm flex-shrink-0`}
                    >
                      <span className="font-label-mono text-label-sm font-black text-black uppercase">
                        {h.level}
                      </span>
                    </div>
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="font-label-mono text-label-mono text-on-surface font-bold">
                        ...
                        <span
                          className={`ml-2 ${getKategoriColor(h.kategori_kemampuan)}`}
                        >
                          {" "}
                    ...
                    <div className="text-right flex-shrink-0">
                      <p
                        className={`font-display text-headline-lg font-black ${scoreColor}`}
                      >
                        {h.total_score}
                      </p>
