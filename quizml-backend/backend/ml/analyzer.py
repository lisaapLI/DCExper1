...
def analyze_quiz(answers: list, questions: list, waktu_detik: int) -> dict:
    ...
    # Hitung konsistensi_score: std deviasi skor per materi (semakin kecil = semakin konsisten)
    import numpy as np
    skor_materi_list = [materi_scores[m] for m in MATERI_LIST]
    konsistensi_score = round(float(np.std(skor_materi_list)), 4)
    ...
    cluster_result = predict_category(
        ...
        konsistensi_score        = konsistensi_score,
    )
